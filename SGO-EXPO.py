...
def sgo_exponential(fitness, max_iter, n, dim, minx, maxx):
    Fitness_Curve = np.zeros(max_iter)
    EXPOBEST = np.zeros(max_iter)
    ...
    while Iter < max_iter:
        for i in range(n):
            Xnew = np.random.exponential(scale=(maxx - minx) / 4, size=dim)
    ...
    return Xbest, Fitness_Curve , Fbest ,  EXPOBEST
...
# Run SGO with Exponential distribution initialization
Xbest_sgo, Fitness_Curve_sgo , Fbest_sgo ,EXPOBEST = sgo_exponential(fitness_function, max_iter, n, dim, minx, maxx)

# Plot fitness curve for SGO with Exponential distribution initialization
plot_fitness_curve("SGO (Exponential)", Fitness_Curve_sgo, f"Fitness Curve - SGO - {function_name} - Exponential")
print(f"Best Fitness Value: {Fbest_sgo}")

print(f"Best Fitness Value for each iteration:")
for values in EXPOBEST:
    print(values)