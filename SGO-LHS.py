...
def lhs_sample(n, dim, minx, maxx):
    # Generate Latin Hypercube samples
    lhs_samples = np.zeros((n, dim))
    for i in range(dim):
        lhs_samples[:, i] = minx + np.random.rand(n) * (maxx - minx)
        np.random.shuffle(lhs_samples[:, i])
    return lhs_samples


def sgo_with_lhs(fitness, max_iter, n, dim, minx, maxx):
    Fitness_Curve = np.zeros(max_iter)
    LHSBEST = np.zeros(max_iter)

    # Use LHS samples for initialization
    lhs_samples = lhs_sample(n, dim, minx, maxx)
    ...
    return Xbest, Fitness_Curve , Fbest, LHSBEST
...
# Run SGO with LHS initialization
Xbest_sgo_lhs, Fitness_Curve_sgo_lhs , Fbest_sgo_lhs ,LHSBEST = sgo_with_lhs(fitness_function, max_iter, n, dim, minx, maxx)

# Plot fitness curve for SGO with LHS
plot_fitness_curve("SGO with LHS", Fitness_Curve_sgo_lhs, f"Fitness Curve - SGO with LHS - {function_name}")
print(f"Best Fitness Value: {Fbest_sgo_lhs}")

for values in LHSBEST:
    print(values)